...
public class TrollSpells : MonoBehaviour {
	...
	private MeleeSystem meleeScript;

	// The spell effects
	public GameObject spell1Effect;
	
	//Boolean vars to check if Spell_1 Button and Spell_2 Button have pressed
	...
	private float manaCost2 = 35f;
	private string manaMess = "";
	
	private float cooldown1 = 10f;
	...
	[RPC]
	void UpdateAttackSpeedEffects(Vector3 position, Quaternion rotation)
	{
		playSpellAnimation(attackSpeedAnimation);
		source.PlayOneShot(spell1Ac);

		GameObject spell1 = (GameObject)Instantiate (spell1Effect, position, rotation);
		spell1.gameObject.transform.parent = this.gameObject.transform;
	}
